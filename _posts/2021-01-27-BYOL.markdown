...
+ $q_\theta(z_\theta)$ 와 $z_\xi'$ 를 $l_2$-normailzation 수행하여 $\bar{q_\theta}(z_\theta) = q_\theta(z_\theta)/\left \| q_\theta(z_\theta) \right \|_2$ 와 $\bar{z_\xi}' = z_\xi' / \left \| z_\xi' \right \|_2$ 를 산출
